...
<%!
    import re
    from pygodot.bindings._generator import python_module_name, remove_nested_type_prefix, CORE_TYPES, clean_signature, clean_return_type
...
cdef class ${class_name}(${class_def['base_class'] or '_Wrapped'}):
    ...
    cdef ${clean_return_type(return_type, forwards, includes, class_name, class_def['base_class'])}${method_name}(
        self${', ' if pxd_signature else ''}${clean_signature(pxd_signature, forwards, includes, class_name, class_def['base_class'])})
    % endfor
