...
env.Append(LIBPATH=config_vars['LIBPL'])
...
if env['target_extension']:
    ...
    static_target_name = static_target_name + config_vars['SHLIB_SUFFIX']

static_library = env.StaticLibrary(target=static_target_name, source=sources)

if env['target_extension']:
    dl_env = env.Clone()
    dl_env.Append(LIBS=[static_library, 'python%(py_version_short)s%(abiflags)s' % config_vars])
    dl_env['SHLIBPREFIX'] = ''
